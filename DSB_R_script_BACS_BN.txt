...
dat.s<-split(dat.ss, dat.ss$PointName)	#split by pt. name, **see 'subset data' above
...
colnames(burn)<-c(1:length(colnames(burn)))	#recode point names of burn into integers as well

dat.s$'73'[,2]	###=dat.s[[p]][,2] in loop###vector of survey dates for point 73
burn[,73]	#vector of burn dates for point 73
p.name[73]	#"L5" is point 73
...
for(p in 1:ncol(burn)){	#burn has 79 columns, 1:79 corresponding to points J1:R9
...
day.v<-dat.s$'73'[,2]	#make a vector for survey dates at point '73' = 'L5'
